...
3. Скачайте файл `MOH-Tools.lua` из репозитория
4. Поместите файл в папку: `%ПУТЬ_К_GTA_SA%\moonloader\`
5. Запустите GTA SA и SAMP
6. В игре введите `/mt` для открытия интерфейса скрипта

## 🔄 Обновление
Скрипт имеет встроенную систему автоматического обновления. Для ручного обновления:
1. Скачайте новую версию файла `MOH-Tools.lua`
2. Замените существующий файл в папке moonloader
3. Введите в игре команду `/lua reload MOH-Tools.lua`
...
- При возникновении ошибок проверьте логи в папке `moonloader/config/MOH-Tools/logs`
